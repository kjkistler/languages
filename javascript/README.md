...
In the example above, the value saved to the `length` property is retrieved from the string, 'Hello'. The program logs 5 to the console, because 'Hello' has five characters in it.

## Built-in Methods

While the length of a string is calculated when an instance is created, a string instance also has *methods* that calculate new information as needed. When these built-in methods are called on an instance, they perform actions that generate output.

Built-in methods are *called*, or used, by appending an instance with a period, the name of the method, and `()`. Consider the examples below:

```
console.log('Hello'.toUpperCase());  // Logs 'HELLO' to the console
console.log('Hey'.startsWith('H'));  // Logs true to the console
```

Let's look at each line separately:

* On the first line, the `.toUpperCase()` method is called on the string instance 'Hello'. The result is logged to the console. This method returns the string in all capital letters, 'HELLO'.
